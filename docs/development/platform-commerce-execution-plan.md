...
- 2026-08-04：**统一设置模块**（用户验收后追加需求，参考 sub2api 设置体系）——`internal/settings` 服务成为运行时配置唯一访问路径：商务节（汇率/告警线）复用统一 `system_settings` key/JSONB 表（新 key `commerce.runtime`，零迁移）；DB 权威 + 热生效（pricing 与 walletalert 改为 provider 注入按次读取，汇率清空即 fail-closed 停用基准定价），环境变量仅首启种子。管理端 `GET/PUT /api/v1/admin/settings/commerce`（L2 审计）。控制台"注册与安全"页归并为"系统设置"三 tab（注册与安全/商务定价/收款渠道），菜单同步更名。测试：设置往返+热生效（改汇率预览价即变、清空即 404 pricing_disabled）、种子仅首启（DB 值胜过环境）、无效值 400、非管理员 403、审计留痕；walletalert/pricing 动态开关用例。全量绿。
