...
> **本仓库于 2026-08-09 正式退役，不再接受功能迭代。**
>
> **名字已交接**：本仓退役后更名为 **e2mapi-archive**，`e2mapi` 这个名字归属新的
> 基座仓库（原 `e2m-platform`，sub2api 深度 fork，本机 `~/桌面/e2mapi`）。所以：
> **2026-08-09 之前的"e2mapi"指本仓，之后的"e2mapi"指那个新仓**；下文与其余历史
> 文档提到的 `e2m-platform` 即今日的 `e2mapi`。
>
> 产品基座已整体迁移到该仓，理由是本仓与它各自维护着一套数据面，
