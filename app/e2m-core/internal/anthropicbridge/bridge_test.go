...
func TestAnthropicToChatCompletionsRequest_BasicText(t *testing.T) {
	...
	// E2M sends max_tokens, not max_completion_tokens: third-party
	// OpenAI-compatible upstreams overwhelmingly implement the former.
	require.NotNil(t, out.MaxTokens)
	require.Equal(t, 1024, *out.MaxTokens)
	require.Nil(t, out.MaxCompletionTokens)
}
...
// E2M does not invent a reasoning effort. sub2api defaulted to "medium" and
// always sent the field; an upstream that does not know it answers 400, and the
// customer never asked for it.
func TestAnthropicToChatCompletionsRequest_ReasoningEffortOmittedUnlessRequested(t *testing.T) {
	req := &AnthropicRequest{
		Model:     "gpt-5.4",
		MaxTokens: 100,
		Messages:  []AnthropicMessage{{Role: "user", Content: json.RawMessage(`"hi"`)}},
	...
	out, err := AnthropicToChatCompletionsRequest(req)
	require.NoError(t, err)
	require.Equal(t, "", out.ReasoningEffort)
