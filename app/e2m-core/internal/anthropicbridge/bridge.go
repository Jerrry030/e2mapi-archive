...
// This file translates between the Anthropic Messages protocol and OpenAI Chat
// Completions in both directions:
//
//	Request:  Anthropic Messages → Chat Completions
//	Response: CC response/chunk  → Anthropic message/events
//
// Every E2M platform upstream is OpenAI-compatible, so a customer speaking the
// Messages protocol is served from the same pool, the same reservation and the
// same failover loop as an OpenAI customer. The package is pure translation: no
// I/O, no store, no accounting. The gateway owns the wire and the money.
//
// The streaming direction carries four behaviors that are easy to get wrong and
// expensive to discover in production, all exercised by the tests in this
// package:
//
//  1. Deferred announcement. A tool's content_block_start must wait for its
//     NAME, because several upstreams stream the id and arguments first.
//     Announcing early emits a nameless tool_use the client cannot dispatch.
//  2. Argument buffering. Fragments that arrive before the announcement are
//     held and flushed as one input_json_delta right after it.
//  3. Name never arrives. At finalize, still-pending tools are announced with
//     an empty name so buffered arguments are not silently lost.
//  4. Empty-argument placeholder. A tool block that streamed no argument delta
//     gets a synthetic "{}" delta before its content_block_stop, because some
//     clients assemble tool input exclusively from deltas.
...
func AnthropicToChatCompletionsRequest(req *AnthropicRequest) (*ChatCompletionsRequest, error) {
	...
	// Sampling params: OpenAI's reasoning models reject temperature/top_p.
	// The name test is a heuristic inherited from sub2api; it is deliberately
	// narrow, because guessing wrong strips parameters the customer set.
	...
	// Only forward a reasoning effort the client actually set. sub2api
	// defaulted to "medium" and always sent the field; a third-party
	// OpenAI-compatible upstream that does not know it answers 400.
	if req.OutputConfig != nil && req.OutputConfig.Effort != "" {
		out.ReasoningEffort = mapAnthropicEffort(req.OutputConfig.Effort)
	}

	// parallel_tool_calls is deliberately NOT set. Anthropic has no equivalent
	// knob, so sending it states a preference the customer never expressed and
	// risks a 400 from upstreams that reject unknown fields.
...
func anthropicToChatMessages(system json.RawMessage, msgs []AnthropicMessage) ([]ChatMessage, error) {
	...
	// System prompt → system message. Both the string and []block encodings
	// are accepted, and every text block is forwarded verbatim.
	if len(system) > 0 {
		text, err := parseAnthropicSystemText(system)
		if err != nil {
			return nil, err
		}
		if text != "" {
...
func chatMessageToAnthropicBlocks(message ChatMessage) []AnthropicContentBlock {
	...
	for _, toolCall := range message.ToolCalls {
		...
		blocks = append(blocks, AnthropicContentBlock{
			Type:  "tool_use",
			ID:    normalizeCallID(toolCall.ID),
