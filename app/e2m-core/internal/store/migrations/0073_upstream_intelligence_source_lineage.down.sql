...
-- Older binaries do not understand source lineage. Preserve every version as
-- an explicit fail-closed cause rather than delete history during downgrade.
UPDATE upstream_intelligence_fact_mutations
...
BEGIN
    ...
    END IF;
    IF target_mutation_kind IN ('quality','collection','link')
